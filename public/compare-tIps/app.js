...
$(function(){
    ...
    getAllMatchData('https://www.scibet.com/').then(function (html) {
        ...
        for (let i = 0; i < matches.length; i++) { 
            ...
            matchData = {
                ...
                prediction_1: Math.round(parseFloat($(matches[i]).find('td:nth-child(9) .bar-success').attr('style').split(':')[1].split('%')[0]) * 100) / 100 >= 75 ? true : false,
                prediction_x: Math.round(parseFloat($(matches[i]).find('td:nth-child(9) .bar-warning').attr('style').split(':')[1].split('%')[0]) * 100) / 100 >= 40 ? true : false,
                prediction_2: Math.round(parseFloat($(matches[i]).find('td:nth-child(9) .bar-danger').attr('style').split(':')[1].split('%')[0]) * 100) / 100 >= 70 ? true : false,
            }
            ...
            if (winPredictionsTitle[i].innerText === 'Win Accumulator') {
                ...
                for (let i = 0; i < matches.length; i++) { 
                    ...
                    matchData = {
                        ...
                        prediction_1: $(matches[i]).find('.tips-card__name-two').text().toLowerCase().includes("home win") === true ? true : false,
                        prediction_x: $(matches[i]).find('.tips-card__name-two').text().toLowerCase().includes("draw") === true ? true : false,
                        prediction_2: $(matches[i]).find('.tips-card__name-two').text().toLowerCase().includes("away win") === true ? true : false,
                    }
    ...
    getAllMatchData('https://www.tips180.com').then(function (html) {
        ...
        for (let i = 0; i < matches.length; i++) { 
            const match = $(matches[i]).find('td').eq(2).text().split(' vs ').length === 2 ? $(matches[i]).find('td').eq(2).text().split(' vs ') : $(matches[i]).find('td').eq(2).text().split(' v ');
...
$('html').on('click', '.load-button_1-a', function() {
...
$('html').on('click', '.load-button_1-b', function() {
    allMatchesTodayFiltered = [];
...
$('html').on('click', '.load-button_x-a', function() {
    allMatchesTodayFiltered = [];
...
$('html').on('click', '.load-button_x-b', function() {
    allMatchesTodayFiltered = [];
    mergeAllMatchesWithAllFiltredMatchces(allMatchesToday, forebet_MatchData, 'prediction_x', true);
    mergeAllMatchesWithAllFiltredMatchces(allMatchesToday, statarea_MatchData, 'prediction_x', true);
    mergeAllMatchesWithAllFiltredMatchces(allMatchesToday, footballpredictionsCom_MatchData, 'prediction_x', true);
...
$('html').on('click', '.load-button_2-a', function() {
    allMatchesTodayFiltered = [];
...
$('html').on('click', '.load-button_2-b', function() {
    allMatchesTodayFiltered = [];
    mergeAllMatchesWithAllFiltredMatchces(allMatchesToday, supatips_MatchData, 'prediction_2', true);
    mergeAllMatchesWithAllFiltredMatchces(allMatchesToday, forebet_MatchData, 'prediction_2', true);
    mergeAllMatchesWithAllFiltredMatchces(allMatchesToday, scibet_MatchData, 'prediction_2', true);
    ...
    for (let i = 0; i < filteredMatches.length; i++) { 
        ...
        if (filteredMatches[i][selector] === condition) {
            ...
            if (findHomeTeam.length > 0 && findAwayTeam.length > 0) {
                ...
                let matchName;
                const matches = findHomeTeam.length > findAwayTeam.length ? findAwayTeam : findHomeTeam;

                if (matches.length > 1) {
                    for (let y = 0; y < matches.length; y++) {
                        const findHomeTeam = matches[y].homeTeam.length >= filteredMatches[i].homeTeam.length 
                            ? matches[y].homeTeam.toLowerCase().indexOf(filteredMatches[i].homeTeam.toLowerCase()) !== -1
                            : filteredMatches[i].homeTeam.toLowerCase().indexOf(matches[y].homeTeam.toLowerCase()) !== -1
                        const findAwayTeam = matches[y].awayTeam.length >= filteredMatches[i].awayTeam.length 
                            ? matches[y].awayTeam.toLowerCase().indexOf(filteredMatches[i].awayTeam.toLowerCase()) !== -1
                            : filteredMatches[i].awayTeam.toLowerCase().indexOf(matches[y].awayTeam.toLowerCase()) !== -1
                ...
                allMatchesTodayFiltered.forEach((match, i) => {
                    if (match.id === matchName.homeTeam + matchName.awayTeam) {
                        duplicate = i;
                        return;
                    }
                })
                ...
                if (duplicate !== null) {
                    ...
                    allMatchesTodayFiltered.push({
                        found: 1,
                        matchTime: matchName.matchTime,
                        homeTeam: matchName.homeTeam,
                        awayTeam: matchName.awayTeam,
                        league: matchName.league,
                        website: [filteredMatches[i].website],
                        odds_1: matchName.odds_1,
                        odds_x: matchName.odds_x,
                        odds_2: matchName.odds_2,
                        id: matchName.homeTeam + matchName.awayTeam,
                    })
                }
            }
        }
